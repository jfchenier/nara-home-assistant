...
class NaraAPI:
    ...
    def get_data(self):
        """
        Fetches all historical activities by triggering a sync with the backend datastore.
        
        Returns:
            dict: A dictionary of all tracked activities keyed by their track_id.
        """
        payload = {
            "data": {
        ...
        res = requests.post(self.CF_URL, json=payload, headers={"Authorization": f"Bearer {self.id_token}"})
        if res.status_code == 200:
            return res.json().get("result", {}).get("trackz", {})
        return {}
                    ...
                    if current_event in ["put", "patch"]:
                        ...
                        if path_val == "/":
                            if isinstance(data_val, dict):
                                for key, item in data_val.items():
    ...
    def _push_payload(self, payload, track_id=None):
        ...
        requests.put(f"{self.DB_URL}{path_trackz}?auth={self.id_token}", json=payload)
        ...
        res2 = requests.put(f"{self.DB_URL}{path_instreamz}?auth={self.id_token}", json=payload)
        
        if res2.status_code != 200:
            raise Exception(f"Failed to push activity to sync queue: {res2.text}")
    ...
    def get_track(self, track_id):
        ...
        res = requests.get(f"{self.DB_URL}{path}?auth={self.id_token}")
        if res.status_code == 200:
            return res.json()
        return None
    ...
    def patch_activity(self, track_id, updates):
        """
        Updates specific fields of an existing track without overwriting it,
        then pushes the full updated track to the instreamz sync queue.
        """
        path_trackz = f"/familyz/{self.family_key}/trackz/{track_id}.json"
        res = requests.patch(f"{self.DB_URL}{path_trackz}?auth={self.id_token}", json=updates)
        if res.status_code != 200:
            raise Exception(f"Failed to patch track: {res.text}")
        ...
        if full_track:
            sync_group = self._generate_id()
            path_instreamz = f"/instreamz/familyz/{self.family_key}/trackz/{self.uid}/{sync_group}/value/{track_id}.json"
            requests.put(f"{self.DB_URL}{path_instreamz}?auth={self.id_token}", json=full_track)
